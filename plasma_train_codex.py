...
def main():
    ...
    cache_dir = '/mnt/linshuijin/ADNI_cache_tau_fp16_3tok'  # ★ 3-token context，与旧 2-token 缓存隔离
    ...
    # ================================================================
    # 2. 预计算模态优化文本特征（Token 2，与 train.py legacy 文本一致）
    # ================================================================
    ...
    print("\n🔧 Plasma 引导模式: context = [plasma_emb, clinical_emb, modality_text_optimized]")
    ...
    for i in range(0, len(clinical_texts), CLINICAL_BATCH):
        ...
        with torch.no_grad():
            ...
            feats = F.normalize(feats, dim=-1)              # L2 归一化，对齐 plasma_emb 尺度
    ...
    # ================================================================
    # 6. 定义 index_transform（Token 0: plasma_emb, Token 1: clinical_emb, Token 2: modality_text）
    # ================================================================
    ...
    shape_diagnostic_printed = False
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for step, data in progress_bar:
            ...
            if not shape_diagnostic_printed and tau_index is not None:
                print(f"🔎 首个 batch tau_index.shape: {tuple(tau_index.shape)}")
                shape_diagnostic_printed = True
