...
import os
_TARGET_GPU_IDS = [0]  # 指定使用的 GPU ID，与下方 device_id 保持一致
...
def main():
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for step, data in progress_bar:
            ...
            # ============ 大显存方案：每个 batch 对所有可用模态联合训练，一次反向传播 ============
            ...
            # 仅在至少有一个模态可用时进行反向传播
            if has_fdg or has_av45 or has_tau:
                scaler.scale(loss).backward()

                # 恢复为未除 accumulation_steps 的值用于日志
                total_loss = loss.item() * accumulation_steps
                loss_fdg_item = loss_fdg.item() if has_fdg else 0.0
                loss_av45_item = loss_av45.item() if has_av45 else 0.0
                loss_tau_item = loss_tau.item() if has_tau else 0.0

            # ============ 梯度累积：每 accumulation_steps 步才更新参数 ============
            if (step + 1) % accumulation_steps == 0 and (has_fdg or has_av45 or has_tau):
            ...
            if log_every > 0 and global_step % log_every == 0:
                ...
                writer.add_scalar("train/loss_fdg", loss_fdg_item, global_step)
                writer.add_scalar("train/loss_av45", loss_av45_item, global_step)
                writer.add_scalar("train/loss_tau", loss_tau_item, global_step)
